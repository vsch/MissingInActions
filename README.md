...
* Fix: handling of `beforeActionPerformed` without corresponding `afterActionPerformed` which
  would cause malfunction of the plugin because it interpreted all subsequent actions as nested
  action invocation. This happened most often when an action caused an exception cancelling all
  subsequent "normal" operations.
